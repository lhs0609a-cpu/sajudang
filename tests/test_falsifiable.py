...
# 센 수 — 아라비아 숫자와 **세는 자리의 한글 수사**.
#
# ★ 처음에는 `\d` 뿐이었습니다. 그래서 「물은 넷이오, 나무는 하나도
#   없소」 를 센 수가 없는 말로 봤습니다. 한글로 적은 개수도 틀릴 수
#   있는 수입니다 — 손님이 만세력을 펴고 세면 다를 수 있습니다.
#
#   다만 지나가는 「하나」 까지 세면 검사가 헐거워집니다. 뒤에 세는
#   자리가 오는 것만 봅니다 — 「셋이오」 「넷뿐」 「둘째」 「하나도 없소」.
#   「한 가지」 「여덟 글자」 는 안 걸립니다.
NUM = re.compile(
    r"\d"
    r"|(?:하나|둘|셋|넷|다섯|여섯|일곱|여덟|아홉|열)(?=이오|뿐|밖에|째)"
    r"|하나도 없")
