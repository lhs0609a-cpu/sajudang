...
def test_ㅣ어간_움직씨는_여로_준다():
    """
    ★ 「스스로를 몰아붙이오」 가 해요체에서 「몰아붙이에요」, 반말에서
      「몰아붙이야」 로 나갔습니다 (2026-09-23).

      이다(것이·방식이)와 어간의 이(붙이·움직이)를 **앞 글자의
      받침**으로 가르는데, 시킴·입음으로 지은 어간은 앞에 받침이
      있어 이름씨로 잘못 셌습니다. ㅣ 어간에 -어가 붙으면 ㅕ로
      줄어듭니다 — 움직이+어 → 움직여.
    """
    for hao, haeyo, banmal in (
            ("그리고 스스로를 몰아붙이오.", "몰아붙여요", "몰아붙여"),
            ("속이 먼저 움직이오.", "움직여요", "움직여"),
            ("적어도 쌓이오.", "쌓여요", "쌓여"),
            ("사람들이 귀를 기울이오.", "기울여요", "기울여"),
            ("발이 묶이오.", "묶여요", "묶여")):
        assert haeyo in V.speak(hao, V.HAEYO), hao
        assert banmal in V.speak(hao, V.BANMAL), hao
    # 이름씨 + 이다는 그대로 — 「방식여요」 가 되면 안 됩니다.
    for hao, haeyo in (("그것이 방식이오.", "방식이에요"),
                       ("이 자리의 쓰임이오.", "쓰임이에요"),
                       ("그게 돈벌이오.", "돈벌이에요")):
        assert haeyo in V.speak(hao, V.HAEYO), hao


def test_묻는_어미_까요는_건드리지_않는다():
    """
    ★ 「무엇부터 끝낼까요?」 가 「끝낼까입니다? · 끝낼까네? ·
      끝낼까예요?」 로 나갔습니다 (2026-09-23). 「끝낼까」 를 이름씨로
      보고 이다를 붙인 것이오. -ㄹ까요는 묻는 어미라 그대로 둡니다.
    """
    for v in V.VOICES:
        got = V.speak("무엇부터 끝낼까요?", v)
        assert got == "무엇부터 끝낼까요?", (v, got)


def test_매개모음_으를_뗀다():
    """
    ★ 「혼자 겪으오」 가 「겪읍니다 · 겪으네 · 겪으지 · 겪으네요」 로
      나갔습니다 (2026-09-23). 받침 뒤의 「으」 는 어간이 아니라
      이음소리라, 다른 결로 갈 때는 떼고 붙여야 하오 — 시키는 말에서
      이미 겪은 자리요(「끊으게」).
    """
    assert V.speak("혼자 겪으오.", V.HAPSYO) == "혼자 겪습니다."
    assert V.speak("혼자 겪으오.", V.HAGE) == "혼자 겪네."
    assert V.speak("마음을 가다듬으오.", V.HAPSYO) == "마음을 가다듬습니다."
    assert V.speak("글을 읽으오.", V.HAEYO) == "글을 읽어요."
    # ★ ㄷ불규칙은 되살릴 수가 없으니 **물러섭니다** — 떼면 「걸습니다」.
    assert "걸습니다" not in V.speak("같이 걸으오.", V.HAPSYO)
