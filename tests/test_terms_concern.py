...
# 낱말 안을 풀지 않는다
# ══════════════════════════════════════════════════════════
def test_낱말_안의_지지는_안_푼다():
    """
    ★ 「아껴지지 않소」 가 「아껴지지(아래에 오는 글자)가 않소」 로
      나갔습니다 (2026-09-19). 「-어지다」 의 부정형이 地支 로 잡힌
      것이오.

      여태 `NOT_AFTER["지지"]` 에 어간 끝 글자를 **손으로 적어** 막고
      있었는데, 새 문장을 쓸 때마다 구멍이 났습니다. seed 를 다 세어
      보니 빠진 것이 여덟 자 더 있었습니다 — 껴 · 따 · 커 · 눠 ·
      해 · 아 · 라 · 너.

      못 풀고 지나가면 풀이가 하나 빠지는 것이고, 낱말 안을 풀면
      **문장이 깨집니다.** 둘 중에는 앞이 낫소.
    """
    from engine import terms as terms_mod
    for s in ("아껴도 아껴지지 않소.", "따지지 않소.", "커지지 않소.",
              "나눠지지 않소.", "좋아지지 않소.", "달라지지 않소.",
              "무너지지 않소.", "따뜻해지지 않소."):
        assert terms_mod.gloss(s, set()) == s, s


def test_낱말_머리의_지지는_푼다():
    """막느라 진짜 용어까지 막으면 그건 고친 것이 아니오."""
    from engine import terms as terms_mod
    for s in ("지지 여덟 글자 중 아래 넉 자요.", "여덟 글자의 지지를 보오."):
        assert '<i class="gl">' in terms_mod.gloss(s, set()), s
