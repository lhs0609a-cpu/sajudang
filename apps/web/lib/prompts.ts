...
export interface PromptEntry {
  ...
  revised?: string | null;
  /*
   * ★ 초상은 **한 장이 아닙니다** (2026-09-06).
   *
   *   같은 사람의 얼굴이 셋입니다 — 늘 쓰는 얼굴 · 짚는 얼굴 ·
   *   누그러뜨리는 얼굴. 그리고 움직이는 것이 둘입니다 — 도는
   *   초상(clip)과 첫 대면 인사(greet).
   *
   *   묶음에는 다 들어 있었는데 **창이 ① 과 ② 만 보여 줬습니다.**
   *   그리는 사람은 창을 열어 복사할 뿐이라, 안 보이면 없는 것과
   *   같습니다. 실제로 표정 둘은 한 장도 안 들어왔습니다.
   */
  moods?: Record<string, { file: string; ko: string; image: string }>
    | null;
  greet?: string | null;
  /*
