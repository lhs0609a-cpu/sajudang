...
export default function RefundHistory({sessionId}:{sessionId:string}){
  ...
  const load=useCallback(async(signal?:AbortSignal)=>{
    ...
    }catch{if(!signal?.aborted)setLoadError('구매 내역을 확인하지 못했어요. 구매가 없다는 뜻은 아닙니다. 잠시 후 다시 확인해 주세요.');}
  ...
  const request=async(o:Order)=>{
    ...
    try{
      ...
      setMessage(data.status==='review_requested'?'계산 오류 확인 요청을 접수했어요. 처리 상태는 이곳에서 확인할 수 있어요.':data.reissue?'전액 환불하고 추가 결제 없이 리포트를 재발행했어요. 기존 리포트에서 다시 읽을 수 있어요.':'전액 환불 처리됐어요. 카드사 반영까지 시간이 걸릴 수 있어요.');
  ...
  return <section className="conversion-card"><h2>구매 내역과 환불</h2>
    {loading&&<p role="status">구매 내역을 확인하고 있어요…</p>}
    ...
    {!loading&&!loadError&&!orders.length&&<p className="conversion-note">아직 구매 내역이 없어요. 다른 기기의 구매는 주문번호로 복원할 수 있어요.</p>}
    {orders.map(o=><div key={o.order_id} className="conversion-card">
      ...
      {o.reissued_from&&<p>계산 오류 확인 후 추가 결제 없이 재발행한 리포트입니다.</p>}
