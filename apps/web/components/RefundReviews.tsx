...
export default function RefundReviews({accessKey,token}:{accessKey:string;token:string}){
  ...
  const load=async()=>{const r=await fetch(`${API_BASE}/v1/admin/refund-reviews`,{headers});if(!r.ok)throw Error('환불 확인 요청을 조회하지 못했습니다.');setRows((await r.json()).requests);};
  ...
  const decide=async(order_id:string,approve:boolean)=>{
    ...
    try{
      ...
      setNotes(n=>({...n,[order_id]:''}));setMessage(approve?'전액 환불 처리했습니다.':'검토 결과를 기록했습니다.');await load();
    }catch(e){setMessage(e instanceof Error?e.message:'처리 실패');}finally{setBusy(false);}
  };
  return <section><h2>계산 오류 환불 확인</h2><p className="sm">승인은 실제 전액 환불을 실행합니다. 고객이 제시한 계산 근거를 확인한 뒤 처리하세요.</p>
    {rows.length===0&&<p>대기 중인 요청이 없습니다.</p>}
