...
export default function ExperimentPanel({accessKey,token}:{accessKey:string;token:string}) {
  ...
  useEffect(()=>{
    ...
    const load = async()=>{
      try {
        ...
        if(!r.ok)throw Error('실험 지표를 불러오지 못했소.');
  ...
  return <section><h2>첫 화면 A/B 실험 · 무료 가치 설명</h2>
    <p className="sm">익명 브라우저에 50:50 고정 배정하오. 가격·상품·결제 조건은 동일하오.</p>
    ...
    {data&&<><div style={{overflowX:'auto'}}><table><thead><tr><th>군</th><th>7일 관찰 완료</th><th>구매자</th><th>전환율 / 95% 구간</th><th>매출</th><th>환불</th><th>순매출</th></tr></thead>
      ...
      <p className="sm">관찰 중 {data.immature}개 브라우저. {data.note} 순매출은 수수료·세금 차감 전이며 재구독 매출은 제외하오.</p></>}
    {data?.sample_ratio_warning&&<p role="alert">실험 배정 비율이 예상과 다르오. 효과 비교 전에 수집 상태를 확인하시오.</p>}
    {data&&<><h3>첫 방문 실제 성능 · 최근 30일</h3><p className="sm">브라우저별 가장 큰 값의 75백분위수이오. 관찰된 브라우저만 포함하오.</p>
