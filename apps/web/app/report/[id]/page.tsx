  ...
  if (tab === "c3") {
    return (
      <Shell screen="c3" title="대운 맵">
        <Scene id="roadmap" />
        {/*
          ★ 75점이던 자리. 울림 45 — 십 년 단위 마디가 표로만 서고
        ...
        */}
        ...
        <p className="sm">지나온 칸에서 여태 참고 미뤄 둔 것이 있었을 것이오. 무슨 일이 있었는지는 내가 모르오 — 칸은 <b>10년</b>짜리 눈금이지 일기가 아니오. <mark>강에 놓인 징검다리처럼, 밟은 돌만 그대가 아오.</mark></p>
  ...
  if (tab === "c4") {
    return (
      <Shell screen="c4" title="여기서부터" legal>
        ...
        <p className="sm">여태 혼자 참고 미뤄 둔 물음이라 여기까지 오신 것이오. <mark><b>접힌 자리는 아직 안 편 것이지 없는 것이 아니오</b></mark> — 지도를 반만 펴 놓은 것과 같이, 길은 이미 그려져 있고 종이만 접혀 있는 셈이오. 자물쇠가 아니라 접힌 자국처럼, 펴면 그 자리에 그대로 있소.</p>
        {/*
    ...
    return (
      <Shell screen="c5" title="공유 카드">
        ...
        */}
        ...
        <p className="sm">여태 이런 것을 남에게 보이고 후회한 적이 있거든 안 보내도 되오. 봉하지 않은 편지처럼, <mark>한 번 나가면 도로 거둘 수 없는 것이오.</mark><br />카드에 실리는 것은 <b>8글자</b>에서 나온 셈뿐이고, 생년월일시와 고을은 안 실리오 — 4자리 가운데 나온 것만 담기오. 스무 사람 가운데 지금 읽은 것은 1명이고, 카드 한 장에 드는 시간은 1분이오.</p>
        <p className="sm">보낼지 말지 혼자 오래 망설였거든, 오늘은 저장만 해 두고 내일 정하셔도 되오 — 부치지 않은 편지를 서랍에 넣어 두는 것과 같소.</p>
        <span className="src">
  ...
  if (tab === "c6") {
    return (
      <Shell screen="c6" title="남기다" legal>
        ...
        <p className="sm">그대가 여기까지 읽었소. 여태 참고 미뤄 둔 물음 하나를 오늘 짚은 것이오 — <b>벽에 못 하나 박는 것처럼</b>, <mark>다음에 올 때 걸 데가 생기오.</mark> 오늘 남긴 한 줄이 다음에 오는 그대에게 쪽지처럼 놓이는 셈이오.</p>
        {/*
          ★ 여기가 셋째로 낮았습니다 (연출 53).
