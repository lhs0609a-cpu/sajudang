...
| T1-3 검증 | ✅ **446건 전량 통과 · skip 0** — 회귀 50건은 sxtwl 없는 독립 계산으로 채워 잠갔습니다. 유파 확인 20건만 바깥에 남음 |
| T1-4 Feature Store | ✅ 오행·신강약·용신·십신·대운·파생값 |
| T2-1 뱅크 로더 | ✅ 훅 5단 조합 · statement_id 부여 |
| T2-2 금지어 필터 | ✅ 조합 단계 + 전 응답 미들웨어 (이중) |
| T2-3 중복률 | ✅ 훅 **0.1%** · 유료 컷 최다 점유 전부 2% 이하 (일진 15.1% → 1.7% · 용신 13.2% → 1.1%) |
| T3-1 DB | ⚠️ 모델 16테이블 · 알렘빅 초기 리비전 · 시드 스크립트 — **Postgres 미기동으로 실행 검증 안 됨** |
| T3-2 엔드포인트 | ✅ chart · hook · report · relay · feedback · daily · pay |
| T3-3 릴레이 엔진 | ✅ **규칙 20개**(전원) · 노출 재순위 · forced · 무료 폴백 · 브레이크 하드코딩 — 최다 1순위 21.4% · **20/20 도달** |
| **추가 입력** | ✅ 결합 축 6종 — 성향 4글자 · 출생지 · 혈액형 · 그림 · 패 · 상대 사주 · 현재 상황. 두 번째 리포트의 새 문장 1.95 → 5.96개. 얼굴 사진만 보류(생체인식정보) |
...
**다음에 할 일**: 만세력 앱 대조 20건(유파가 갈리는 자리). `.\dev.ps1 plan`.
그 다음은 에셋과 PG 키·알림 채널입니다.
