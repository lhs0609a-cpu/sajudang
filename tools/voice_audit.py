...
#   다섯 말투 어디에서도 그 꼴 그대로 씁니다. 「인가」 를 이미 빼 둔
#   것과 같은 자리입니다.
#
#   늑대가 안 왔는데 늑대라고 외치는 자는 곧 아무도 안 믿습니다.
# ★ 어미처럼 보이는 **이름씨 꼬리** (2026-09-24).
#   「오늘 바꿀 한 가지」 의 「가지」 가 반말 종결 「…지」 로 잡혀,
#   표지판 한 줄이 「반말로 쓴 원문」 으로 찍혔습니다. 표지판은 말이
#   아니니 어느 말투에도 안 걸립니다 — 자가 먼저 알아봐야 합니다.
_NOT_ENDING = ("는지", "ㄴ지", "은지")

# ★ 어미처럼 보이는 **셈 이름씨** (2026-09-24).
#   표지판 「오늘 바꿀 한 가지」 의 「가지」 가 반말 종결 「…지」 로 잡혀,
#   말이 아닌 줄이 「반말로 쓴 원문」 으로 찍혔습니다. 다만 「같이 가지」
#   는 정말 반말이니 **수를 세는 꼴**만 뺍니다 — 앞에 수나 「몇·여러」 가
#   붙은 「가지」요.
_COUNT_NOUN = re.compile(
    r"(?:한|두|세|네|다섯|여섯|일곱|여덟|아홉|열|몇|여러|모든|\d+)\s*가지$")

# ★ 하게체 「…게」 로 잡히는 **이름씨** (2026-09-24).
#   제목 「괜찮다고 답한 뒤에야 느껴지는 무게」 가 하게체 문장으로
#   잡혀서, 하오체 캐릭터 열다섯 명이 「남의 말투를 썼다」 고 찍혔습니다.
#   무게는 이름씨요 — 어미가 아니오.
_NOUN_GE = ("무게", "기게", "지게")
...
def ending_of(sent: str) -> str:
    ...
    if not w:
        return "기타"
    if w.endswith(_PARTICLE) or w.endswith(_NOT_ENDING):
        return "기타"
    if _COUNT_NOUN.search(w) or w.endswith(_NOUN_GE):
        return "기타"
    # ★ 「해요」 하나만 따로 봅니다.
