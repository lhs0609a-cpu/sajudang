...
import dropout_sim as D                            # noqa: E402
...
#   끼우고 다시 돌리세요.
#
#   각 칸: 화면 id → (비관, 기준, 낙관, 그 화면에서 손님이 하는 일)
#
# ★ 화면 **차례는 여기 안 적습니다.** 옛날에 여기 열한 줄을 손으로 박아
#   두었는데 화면 코드가 바뀌어 있었습니다 — 이름(a2)과 성향 넉 자(a4b)가
#   훅 뒤로 내려갔는데도 이 표는 둘을 여전히 명식 앞에서 곱하고 있었고,
#   그래서 값을 치르는 사람을 1.28배 적게 냈습니다. 차례는
#   `dropout_sim.live_path()` 가 `app/page.tsx` 를 걸어서 찾습니다.
RATES = {
    "a1":  (0.35, 0.55, 0.72, "대문에서 첫 버튼을 누른다. 여기가 가장 크게 샙니다"),
    "a2":  (0.80, 0.88, 0.94, "이름을 적거나 그냥 넘어간다. 빠져나갈 이유가 적은 화면"),
    "a5":  (0.78, 0.87, 0.93, "여섯 중 하나를 고른다. 감정으로 여는 질문이라 잘 걷힙니다"),
    "a3":  (0.62, 0.75, 0.85, "년·월·일 + 성별. 이 흐름에서 가장 무거운 화면"),
    "a4":  (0.82, 0.90, 0.95, "「모르오」가 크게 있어 막히지 않는 화면"),
    "a4b": (0.80, 0.89, 0.94, "열여섯 칸이지만 「모르오」가 위에 있음"),
    "a6":  (0.86, 0.92, 0.96, "여덟 글자를 받은 직후. 여기서 나가면 이상한 자리"),
    "a7":  (0.55, 0.70, 0.82, "훅 다섯 단을 끝까지 읽고 다음을 누른다"),
    "d0":  (0.30, 0.45, 0.60, "무료 6단을 읽고 값 고르는 화면으로 간다"),
    "d1":  (0.18, 0.30, 0.44, "목패를 고르고 「이걸로 하겠소」를 누른다"),
    "d2":  (0.55, 0.70, 0.82, "PG 결제창에서 실제로 값을 치른다"),
}


def steps() -> list:
    """살아 있는 화면 차례로 (이름, 비관, 기준, 낙관, 뜻) 을 만든다."""
    path = D.live_path() + D.PAY_PATH          # a1…a7 + d0 d1 d2 d3
    rows = []
    for i, sid in enumerate(path[:-1]):
        if sid not in RATES:
            continue
        nxt = path[i + 1]
        name = "%s %s → %s" % (sid, SS.KO.get(sid, sid), SS.KO.get(nxt, nxt))
        rows.append((name,) + RATES[sid])
    return rows
