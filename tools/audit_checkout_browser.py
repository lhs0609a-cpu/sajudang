...
OUT = ROOT / "out/audit-20260909/browser"
OUT.mkdir(parents=True, exist_ok=True)
WEB = "https://sajudang-three.vercel.app"
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        for width, answer in [(320, "neutral"), (390, "yes"), (390, "no"), (1920, "neutral")]:
            ctx = browser.new_context(viewport={"width": width, "height": 844}, reduced_motion="reduce")
            ctx.add_init_script("localStorage.setItem('sd.sound','off'); localStorage.setItem('sajudang-session',JSON.stringify({state:{admin:false,adminSet:true},version:0}));")
            page = ctx.new_page()
            ...
            try:
                ...
                page.locator(".conversion-checkout .vow").wait_for(timeout=30000)
                snapshot("d1")
                text = page.locator("body").inner_text()
                results.append({"width": width, "answer": answer, "reached": "d1", "screens": snapshots,
                    ...
                    "retry_message": "잠시 후 결제를 다시 시도해 주시오" in text,
