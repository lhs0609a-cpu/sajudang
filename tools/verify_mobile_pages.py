...
OUT=ROOT/'out/mobile-20260909/browser';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ...
 with sync_playwright() as pw:
  ...
  for width,height in ((320,568),(360,740),(390,844),(430,932)):
   ctx=browser.new_context(viewport=dict(width=width,height=height),is_mobile=True,has_touch=True,device_scale_factor=1,reduced_motion='reduce')
   ...
   for name,path in ROUTES:
    ...
    try:
     ...
     info['smallInputs']=page.locator('input:not([type=hidden]):not([type=checkbox]):not([type=radio]),select,textarea').evaluate_all('(els)=>els.filter(e=>e.getBoundingClientRect().width>0 && parseFloat(getComputedStyle(e).fontSize)<16).length')
     info['smallButtons']=page.locator('.phone button,.phone .top .tb').evaluate_all('(els)=>els.filter(e=>e.getBoundingClientRect().width>0 && e.getBoundingClientRect().height<43.5).map(e=>({text:e.textContent,height:e.getBoundingClientRect().height}))')
     if name=='entry':
      box=page.locator('.gatecopy .btn').first.bounding_box()
      info['firstActionBottom']=box['y']+box['height']
      assert info['firstActionBottom']<=height, 'entry action below viewport'
     assert not info['smallInputs'], 'input font smaller than 16px'
     assert not info['smallButtons'], 'button touch height below 44px'
     result=dict(width=width,viewportHeight=height,page=name,errors=list(errors),**info)
