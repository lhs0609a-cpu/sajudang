...
const ORDER = [
  ["1부 · 진입 — 첫 화면부터 명식 세우기까지", [
    ...
    ["desk", "a2 이름을 적다"],
    ["ink", "a3 날·고을 — 붓끝이 닿고 먹이 번지는 종이"],
    ["room", "a4 때를 묻다 — 실내·병풍·주렴"],
    ["mirror", "a4b 성향 4글자 — 넉 자와 여덟 글자를 맞대다"],
    ["fork", "a5 걸리는 것 — 갈림길"],
    ["altar", "a6 글자가 서다 — 편 종이 · 움직이는 붓 · 목패 넷"],
    ["facing", "a7 도령이 말하다 — 마주앉은 자리"],
  ]],
  ["2부 · 진열대 — 캐릭터 고르기", [
    ["hall", "b2 스무 사람 — 진열대 들머리"],
    ["seat", "b3 그 사람의 자리 — 앉은 뒷모습"],
    ["shelf", "b1 진열대 — 걸린 목패 다섯"],
  ]],
  ["3부 · 리포트", [
    ["scroll", "c1 표지 두루마리 · c7 분석지"],
    ["unbind", "c2 본문 여는 자리 — 끈을 풀다"],
    ["roadmap", "c3 대운 맵 — 상 위에 편 긴 종이"],
    ["fold", "c4 페이월 — 접힌 데"],
    ["cardbg", "c5 공유 카드 — 패 뒷면"],
    ["wall", "c6 후기 벽 · r1 다녀간 사람들"],
  ]],
  ["4부 · 결제", [
    ["oldpaper", "d0 값 없이 한 겹 더 · c4 안 파는 자리"],
    ["tray", "d1 어디까지 볼지 — 목패 셋"],
    ["door", "d1b 엿보기 — 접힌 자리에 얹은 손"],
    ["coin", "d2 값을 치르다 — 셈한 종이 · 목패 · 엽전"],
    ["untie", "d3 열렸소 — 매듭을 풀다"],
  ]],
  ["5부 · 릴레이", [
    ["handle", "h1 이어지다 — 문고리"],
...
P();
P(bar("#"));
P("  붙임 3 · 이름과 그림이 어긋나 있는 폴더");
P(bar("#"));
P();
P("  02 접힌 자리 · 얹은 손 (public/scene/door/)");
P();
P("    한때 '열리는 문' 이었고 아무 화면도 안 불렀습니다. 지금은");
P("    d1b 엿보기가 부릅니다 — 폴더 이름만 door 로 남았습니다.");
P("    그림은 문이 아니라 '접힌 두루마리에 얹은 손' 이오. 차례의");
P("    4부에 실려 있으니 그대로 뽑으시오.");
P();
