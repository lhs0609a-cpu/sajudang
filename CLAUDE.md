...
**회귀 케이스 50건의 기대값 채우기.** 여덟 글자가 맞는지 아직 외부와
대조하지 않았습니다. 이게 틀리면 나머지가 전부 무의미합니다.

```powershell
.\dev.ps1 plan               # 무엇부터 볼지 (1순위 20건이면 충분)
...
```
손으로 옮겨 적지 마세요. `fill` 이 대조까지 합니다. 유파가 갈리는
자리(조자시·절입 기준)는 도구가 짚어 줍니다 — 다르다고 곧 버그가 아닙니다.
