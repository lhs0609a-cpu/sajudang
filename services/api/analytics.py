...
EVENTS = {
    ...
    "concern_pick",    # 진열대에서 고민을 골랐다 (a5)
                       # ★ **고른 갈래는 안 싣습니다** — `topic_ask` 와 같은 규칙.
                       #   여태 「골랐다」는 사실조차 안 남아, 고민 화면에서
                       #   몇이 돌아서는지 알 수 없었습니다 (docs/45 §7-2).
    "hour_help",       # 태어난 시각을 집에 물어보러 갔다 (a4)
    "topic_ask",       # 물으신 자리가 되묻는 것에 답했다 (docs/40)
                       # ★ 고른 **갈래는 안 싣습니다.** 화면 이름만.
                       #   짝사랑인지 이별인지는 그 사람에 대한 것이라
                       #   계측에 실으면 준식별자가 됩니다 (CLAUDE.md).
    "tier_view",       # 값 고르는 화면
    "tier_pick",       # 티어를 골랐다
    "pay_start",       # 결제창으로 갔다
    "pay_done",        # 결제가 끝났다
