...
FACE = [
    _option("oval", "타원형", "face-oval.webp", "광대에서 턱으로 완만하게 좁아짐", "부드럽게 좁아지는 얼굴선을 「잘 맞춰 줌」의 표시로 읽어 보겠소. 여러 사람 말을 들을 때도 내 기준 하나는 남겨 두시오."),
    _option("round", "둥근형", "face-round.webp", "가로·세로 길이가 비슷하고 윤곽이 둥긂", "둥근 얼굴선을 「사람과 잘 어울림」의 표시로 읽어 보겠소. 편하게 지내고 싶은 마음과 혼자 쉬고 싶은 마음을 함께 살펴보시오."),
    _option("square", "네모형", "face-square.webp", "이마와 턱 너비가 비슷하고 턱 모서리가 각짐", "각진 얼굴선을 「기준을 지킴」의 표시로 읽어 보겠소. 꼭 지킬 원칙 하나와 바꿔도 되는 방법 하나를 나눠 보시오."),
    _option("long", "긴 타원형", "face-long.webp", "세로로 길고 턱 끝은 둥긂", "길게 이어지는 얼굴선을 「깊이 생각함」의 표시로 읽어 보겠소. 오래 생각한 일이 있다면 작게 시작할 행동 하나도 정해 보시오."),
    _option("heart", "역삼각형", "face-heart.webp", "이마가 넓고 턱 끝으로 많이 좁아짐", "위가 넓고 아래가 모이는 얼굴선을 「생각이 잘 떠오름」의 표시로 읽어 보겠소. 떠오른 생각 중 지금 해 볼 하나를 고르시오."),
    _option("diamond", "마름모형", "face-diamond.webp", "광대가 가장 넓고 이마와 턱이 좁음", "가운데가 도드라지는 얼굴선을 「한곳에 마음을 모음」의 표시로 읽어 보겠소. 눈에 띄는 일과 정말 중요한 일이 같은지 살펴보시오."),
    _option("triangle", "삼각형", "face-triangle.webp", "이마보다 아래턱이 넓음", "아래가 넓은 얼굴선을 「든든한 밑받침」의 표시로 읽어 보겠소. 익숙해서 지키는 것인지, 아직 필요해서 지키는 것인지 스스로 물어보시오."),
    _option("rectangle", "긴 네모형", "face-rectangle.webp", "세로로 길고 양옆과 턱이 각짐", "길고 각진 얼굴선을 「오래 이어 감」의 표시로 읽어 보겠소. 끝까지 해 나갈 일과 중간에 고칠 일을 따로 적어 보시오."),
]
...
FEATURES = {
    "eyes": ("눈", [
        ...
        ("long", "가로로 긴 눈", "양끝 사이 길이가 김", "멀리 봄"),
        ("round", "둥근 눈", "눈의 위아래 곡선이 둥긂", "호기심"),
        ("up", "눈꼬리가 올라간 눈", "바깥 눈꼬리가 안쪽보다 높음", "굳은 마음"),
        ("down", "눈꼬리가 내려간 눈", "바깥 눈꼬리가 안쪽보다 낮음", "공감"),
    ]),
    "brows": ("눈썹", [
        ("straight", "일자 눈썹", "높이 변화가 작은 직선", "한결같음"),
        ("arched", "활 모양 눈썹", "가운데가 높게 휘어진 곡선", "표현"),
        ("thick", "두꺼운 눈썹", "눈썹 띠의 폭이 넓음", "밀고 나감"),
        ("thin", "가는 눈썹", "눈썹 띠의 폭이 좁음", "세심함"),
        ("long", "긴 눈썹", "눈보다 바깥으로 길게 이어짐", "사람을 이어 줌"),
        ("short", "짧은 눈썹", "눈보다 짧게 끝남", "선택"),
    ]),
    "nose": ("코", [
        ("high", "높은 콧대", "옆모습에서 콧대가 앞으로 높이 나옴", "방향"),
        ("low", "낮은 콧대", "옆모습에서 콧대가 얼굴 가까이에 있음", "맞춰 감"),
        ("wide", "넓은 콧방울", "정면에서 코 아래 너비가 넓음", "받아들임"),
        ("narrow", "좁은 콧방울", "정면에서 코 아래 너비가 좁음", "가지런함"),
        ("round", "둥근 코끝", "정면에서 코끝이 둥글게 보임", "넉넉함"),
        ("long", "긴 코", "두 눈썹 사이에서 코끝까지의 길이가 김", "계획"),
    ]),
    "mouth": ("입", [
        ("full", "도톰한 입술", "위아래 입술 두께가 두꺼움", "나눔"),
        ("thin", "얇은 입술", "위아래 입술 두께가 얇음", "아껴 씀"),
        ("wide", "가로로 넓은 입", "입 양끝 사이 거리가 넓음", "주고받는 말"),
        ("narrow", "가로로 좁은 입", "입 양끝 사이 거리가 좁음", "깊은 생각"),
        ("up", "입꼬리가 올라간 입", "입 양끝이 중앙보다 높음", "북돋움"),
        ("down", "입꼬리가 내려간 입", "입 양끝이 중앙보다 낮음", "신중함"),
    ]),
    "jaw": ("턱", [
        ("round", "둥근 턱", "아래 윤곽이 둥근 곡선", "부드러움"),
        ("square", "각진 턱", "양쪽 아래턱 모서리가 뚜렷함", "기준"),
        ("pointed", "뾰족한 턱", "턱 끝이 좁고 뾰족함", "한곳에 모음"),
        ("wide", "넓은 턱", "아래턱의 가로 폭이 넓음", "든든함"),
        ("narrow", "좁은 턱", "아래턱의 가로 폭이 좁음", "가려냄"),
        ("long", "긴 턱", "입 아래부터 턱 끝까지 김", "오래 감"),
    ]),
}
...
BODY_STATES = [
    ...
    _option("tired", "쉽게 지칩니다", "tired.webp", reading="하루 중 유난히 지치는 때와 그전에 한 일을 적어 보시오. 지치는 것을 마음이 약해서라고 여기지 마시오."),
    _option("sleep", "잠이 신경 쓰입니다", "sleep.webp", reading="잠자리에 든 시각과 깬 시각, 낮 동안 느낀 몸 상태를 함께 돌아보시오."),
    _option("rest", "쉬어도 여유가 없습니다", "heal.webp", reading="쉬는 동안에도 마음에 남은 일을 적어 보시오. 그리고 오늘 마칠 일과 다음으로 넘길 일을 나눠 보시오."),
]
...
SCENES = {
    ...
    "jeokhyeol": ("끌리는 사람이 생기면 어찌하오?", [
        _scene("approach", "제가 먼저 다가갑니다", "meet", "먼저 다가간 뒤 상대의 답을 기다릴 틈도 남겨 보시오. 내가 끌리는 것과 상대가 좋다고 하는 것은 따로 확인해야 하오."),
    ...
    "seoyeok": ("살거나 일할 곳을 어디로 넓히려 하오?", [
        _scene("move", "멀리 이주를 생각합니다", "move", "옮기려는 이유와 그곳의 생활 형편을 각각 적어 보시오. 태어난 곳으로 셈한 사주가 새 지역의 실제 형편을 대신 알려 주지는 않소."),
    ...
    "hwagyeong": ("다툼은 어느 장면에서 커지오?", [
        ...
        _scene("boundary", "거리를 정하고 싶습니다", "part", "괜찮은 행동과 어려운 행동을 구체적으로 나눠 보시오. 내가 어디까지 괜찮은지 말하는 것부터 시작하오."),
    ]),
    "haengsu": ("돈 문제는 지금 어디에 걸렸소?", [
    ...
    "nopa": ("갈림길에서 어느 쪽을 생각하오?", [
        ...
        _scene("leave", "다른 길로 가고 싶습니다", "road", "떠나고 싶은 이유와 가고 싶은 곳의 이유를 나눠 보시오. 옮겨 갈 곳의 실제 형편도 같이 보시오."),
        _scene("pause", "잠시 쉬며 생각하고 싶습니다", "heal", "쉰 뒤 다시 살펴볼 질문 하나를 남겨 두시오. 모든 결정을 오늘 마칠 필요는 없소."),
    ]),
    "dongja": ("오늘은 어떤 한 잔이면 좋겠소?", [
        _scene("rest", "조용히 쉬고 싶습니다", "heal", "오늘 더하지 않아도 될 일 하나를 골라 보시오. 오늘은 잠깐 쉬어 가도 되오."),
...
def face_cut(f, payload):
    ...
    for key, (label, options) in FEATURES.items():
        ...
        labels.append(text)
        ids.append(f"{key}={id}")
        rows.append(f'<p><b>{label} · {text}</b> — 여기서는 <b>{symbol}</b>의 표시로 읽소. '
                    f'요즘 {symbol}이 도움이 된 때와 짐이 된 때를 하나씩 떠올려 보시오.</p>')
    if not labels:
        raise VisualInputError("얼굴형이나 부위 특징 중 하나는 골라 주시오.")
    html = ('<p class="sm">고르신 그림으로 성신당이 지어 본 창작 관상 풀이요. '
            '얼굴만 보고 진짜 성격·건강·앞날을 정하지 않소.</p>' + "".join(rows) +
            '<p>이 가운데 요즘 겪는 일과 맞는 것 하나만 골라 보시오. '
            '안 맞는 풀이는 버려도 되오. 아래 사주 풀이는 생년월일로 따로 셈한 것이오.</p>')
    return _cut("face", "그림으로 고른 얼굴 풀이", "직접 고른 특징 · " + " · ".join(labels),
