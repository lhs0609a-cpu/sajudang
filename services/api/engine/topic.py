...
# ── 물은 자리인 줄 알아볼 수 있는 꼴 ──────────────────────────
#
# ★ 이 표는 `tools/topic_reach.py` 에 있었습니다 (2026-09-20 · docs/45).
#   재는 자만 들고 있고 **엔진은 몰랐습니다.** 그래서 무료로 열 컷을
#   고를 때 「물은 자리를 딛는 컷」을 가릴 수가 없었고, 돈을 물은
#   사람에게 「뿌리가 있는가」 가 맛보기로 나갔습니다. 자가 아는 것을
#   엔진이 모르면, 엔진은 그 자를 통과할 수 없습니다.
#
#   도구는 이제 여기서 가져다 씁니다 — 자가 둘이면 언젠가 갈립니다.
WORDS = {
    ...
    # ★ 「일을」 「일이」 는 뺍니다 (2026-09-17).
    #
    #   이 집은 「일」 을 **그 문제**라는 뜻으로도 씁니다 — 「물으신
    #   일을 보는 글자」 「그 일이 늘 어디서부터」. 그걸 직장 얘기로
    #   세니, 방향을 물어도 「일 말이 가장 많소」 가 나왔습니다.
    #   한 낱말이 두 뜻이면 **한쪽만** 세야 하오.
    "work": r"일자리|일터|직장|회사|상사|동료|직업|이직|승진|업무|"
            r"일하|일머리|일에서|맡은 일|하는 일|"
    ...
    # ★ 말투에 걸리는 꼴을 쓰면 안 됩니다 (2026-09-20 · docs/45).
    #
    #   「사람이오」 라고 적어 두었더니 **하오체에서만** 세어졌습니다.
    #   `voice.speak` 가 합쇼체 캐릭터에서 「사람입니다」 로 갈아 끼우면
    #   같은 글이 딴 얘기로 찍힙니다 — 화경(합쇼체)의 무료 컷이 사람을
    #   물었는데 「사람 얘기가 없다」 로 잡혔습니다. 자가 말투를 타면
    #   그 자는 조용히 거짓말하오.
    #
    #   그래서 **어미 앞까지만** 봅니다. 「사람이 많다」 같은 주격은
    #   안 세고(뒤에 종결이 와야 함), 「사람이오·사람입니다·사람이네」
    #   는 한 꼴로 셉니다.
    #   ★ 합쇼체는 「이 + ㅂ니다」 가 **입니다** 로 줄어듭니다 — 「사람이」
    #     로는 안 잡히오. 줄어든 꼴을 따로 답니다.
    "people": r"사람 사이|사람들|사람과|사람이(?=오|네|다|야|에요|예요|랍)|"
              r"사람입니|사람으로|사람을|사람 문제|"
...
def mentions(text: str, concern: Optional[str]) -> bool:
    """이 글이 물으신 자리를 한 번이라도 부르는가."""
    if not concern or not text:
        return False
    pattern = WORDS.get(concern)
