...
# 홍염(紅艶) — 일간 기준. 확정표 docs/14 §2.
#
# ★ 도화와 결이 가깝되 보는 자리가 다릅니다. 도화는 삼합(년지·일지)
#   에서 보고, 홍염은 **일간**에서 봅니다. 그래서 둘이 같이 앉기도
#   하고 하나만 앉기도 합니다.
#
# ★ 다른 유파: 乙을 申으로, 丙을 午로 보는 표가 있습니다.
...
#: 공개 이름 — 사전(engine/dict)이 **같은 표**를 봅니다. 두 벌이 되면
#: 리포트와 사전이 다른 말을 합니다.
SAMHAP = _SAMHAP
...
PALACE = {
    ...
    "월주": {"who": "부모·형제", "also": "자란 집·직장", "age": "16~30세"},
    "일주": {"who": "자신·배우자", "also": "일지는 배우자를 보는 글자", "age": "31~45세"},
    "시주": {"who": "자식·아랫사람", "also": "늘그막·거둔 것", "age": "46세~"},
}
...
def find(chart) -> list:
    ...
    # ── 홍염 (일간 기준) ──
    #
    # ★ 양인과 같은 자리에서 봅니다 — 일간으로 지지를 훑습니다.
    hy = HONGYEOM.get(chart.day_gan)
    if hy:
        at = _hit_pillars(chart.pillars, hy)
        if at:
            out.append({"key": "hongyeom", "name": "홍염", "hanja": "紅艶",
                        "kind": "특수", "at": at, "target": hy})
