...
# ══════════════════════════════════════════════════════════
NAMES = {
    "own": "문장 차별도", "ground": "근거 표기", "sting": "구체 표현",
    "worth": "가격·분량 관계", "read": "문장 길이·용어 풀이", "pull": "화면 문구 규칙",
}
ASK = {
    "own": "고정 표본의 고민·명식별 단어와 컷이 얼마나 다른가",
    "ground": "출처와 계산 근거 표현이 얼마나 포함되어 있는가",
    "sting": "구체적인 장면·행동 표현이 얼마나 포함되어 있는가",
    "worth": "상품 가격과 글 분량이 함께 증가하는가 (만족도 아님)",
    "read": "긴 문단과 풀이 없는 용어의 비율은 얼마인가",
    "pull": "화면 문구가 사전에 정한 표현 규칙에 얼마나 부합하는가",
}
GRADE = [
    (90, "규칙 충족 높음", "자동 검사 결과입니다. 실제 사용자 평가는 별도로 확인하세요."),
    (80, "일부 점검 필요", "낮은 축의 표본 문장을 확인하세요."),
    (70, "문장 검토 필요", "문장과 근거를 직접 검토하세요."),
    (55, "집중 검토 필요", "낮은 축의 원문과 상품 구성을 확인하세요."),
    (0, "검사·콘텐츠 점검", "표본 생성 실패와 원문 문제를 함께 확인하세요."),
]
...
def score() -> dict:
    ...
    return {
        ...
        "total": total,
        "version": 2,
        "coverage": {"birth_profiles": len(_samples()), "lens_id": LENS,
                     "concerns": len(CONCERNS), "reports": sum(len(p) for _, p in _samples())},
