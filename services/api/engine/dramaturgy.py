...
# ★ 한글 수도 수요 (2026-09-17).
#
#   무딘 자리를 갈라 보니 **전부 반증가능 0점**이었습니다 — 용신 ·
#   없는 것 · 위로 · 이번 주 · 얼굴. 그런데 그 컷들을 펴 보면
#   「겉에 쇠는 셋이오, 불은 하나도 없소」 라 적혀 있습니다. 세는
#   말이 분명히 있는데 자가 못 본 것이오. 이 집은 수를 한글로도
#   적습니다 (`engine/bank.count_word`). `\d` 만 보면 그 절반을
#   못 봅니다 — `tools/easy_audit` 이 오늘 같은 자리를 고쳤습니다.
#
#   「하나도 없소」 도 셉니다. 0을 못 박은 말이라 틀릴 수 있소 —
#   손님이 만세력을 펴고 대 보면 바로 갈립니다.
FALSIFIABLE = re.compile(
    r"\d+\s*(?:개|자리|살|세|년|해|번|명|컷|글자|자|시|분|%)|"
    r"(?:하나|둘|셋|넷|다섯|여섯|일곱|여덟|아홉|열|스물|서른|마흔|쉰|"
    r"예순|일흔|여든|아흔)\s*(?:개|자리|살|해|번|명|컷|글자|자)|"
    r"하나도 없|한 자도 없|한 번도 없|"
...
#
# ★ 표지만 세고 있었습니다 — **그림을 안 셌습니다** (2026-09-17).
#
#   손님이 말했습니다: "글에 비유가 적어서 공감이 잘 안가, 너무 다
#   추상적이야." 그런데 이 자는 스물여덟 화면에 95~100을 주고
#   있었습니다. 어느 쪽이 맞는지 표지가 걸린 자리를 뽑아 세어 봤습니다 —
#
#       같이   162번 중 거의 전부가 「함께」 라는 뜻이었소
#              「여럿이 같이 할 때」 「그 짐을 같이 들어 줄 사람」
#       셈이    75번 중 상당수가 「…한 셈이 되오」 라는 말버릇이오
#
#   비유에는 두 쪽이 있습니다 — **표지**(처럼·같소)와 **그림**(무엇처럼).
#   이 자는 왼쪽만 세고 있었으니, 그림 없이 표지만 있는 글도 만점이었소.
#   손님이 맞았습니다.
#
#   더 나쁜 것은 이 자가 `flavor._enough_fig` 에도 쓰이고 있었다는
#   것입니다. 컷 어딘가에 「같이」 가 묻혀 있으면 집은 「그림이 이미
#   있다」 고 판단해 **그림을 영영 안 달았습니다.** 자가 틀리면 글이
#   안 고쳐지는 게 아니라, 고칠 기회가 막힙니다.
#
#   그래서 이제 **한 문장 안에 표지와 몸이 같이** 있어야 셉니다.
FIGURE = re.compile(r"처럼|같이|마치|셈이오|셈입니다|셈이|빗대|듯|"
                    r"[과와] 같소|같은 것이오|같은 것이라|것과 같")

# ── 그림의 몸 — 눈에 보이고 손에 잡히는 것 ────────────────────────
#
# ★ 오늘 물건과 옛 물건을 갈라 둡니다. 둘 다 그림이지만, 옛 물건만
#   쌓이면 손님은 「옛날 얘기」로 읽지 「내 얘기」로 안 읽습니다.
#   이 집의 말투는 옛말이라 그릇을 통째로 바꿀 수는 없고, 다만
#   **얼마나 기울었는지**는 알고 있어야 합니다 (tools/like_me.py).
THING_NOW = re.compile(
    r"통장|월급|카드|영수증|장바구니|냉장고|서랍|책상|의자|가방|주머니|"
    r"휴대폰|전화|문자|메시지|알림|엘리베이터|계단|버스|지하철|택시|"
    r"자전거|신호등|우산|외투|옷장|겉옷|속옷|신발|밑창|이불|베개|알람|"
    r"시계|달력|수첩|거울|돋보기|안경|가계부|비밀번호|사진|영수|"
    r"컵|물통|그릇|수저|냄비|프라이팬|가스불|수도꼭지|세탁기|청소기|"
    r"먼지|빨래|설거지|쓰레기|자물쇠|열쇠|문고리|창문|커튼|전등|전구|"
    r"콘센트|배터리|용수철|고무줄|테이프|나사|망치|가위|바늘|실밥|"
    r"지도|줄자|눈금|풍선|비눗방울|기차|선로|차표|시간표|이정표|"
    r"약[ ,.을은]|소금|설탕|국[ ,.이은을]|밥상|반찬|장을 보|장보|"
    r"톱니|바퀴|사다리|연장통|공구|못[ ,.을이]|볼트|계단참|손잡이|"
    r"책[ ,.을이은]|쪽지|공책|연필|지우개|종이|봉투|상자|"
    r"방[ ,.을이은]|부엌|마당|현관|담장|바닥|천장|지붕|계량|저울|됫박|"
    # ★ 눈에 보이는데 표에 없던 것들 (2026-09-17) — 「동전의 앞면과
    #   뒷면」 「문을 열어 두고」 「짐이 어느 쪽으로」 가 그림으로
    #   안 세어졌습니다. 표지만 있고 몸이 없다고 잡힌 자리요.
    r"동전|지폐|문[ ,.을이은]|짐[ ,.을이은]|옷[ ,.을이은]|수건|빗자루")
THING_OLD = re.compile(
    r"곳간|가마|나룻배|돛|삿갓|도롱이|장터|주막|우물|두레박|"
    r"아궁이|굴뚝|부뚜막|가마솥|절구|맷돌|호미|쟁기|멍석|고삐|수레|"
    r"짚신|베틀|물레|숯|화로|등잔|호롱|부싯돌|대장간|풀무|모루|"
    r"논밭|무논|볏단|장작|땔감|씨앗|모종|접붙|밭고랑|봇물|도랑|둑|"
    r"성벽|툇마루|섬돌|기와|서까래|나룻|길쌈|가마니|바리|창고|대문|"
    r"바닥돌|살림살이|이름표|지명|별명")
THING_LIVE = re.compile(
    r"물길|물빛|바람|햇빛|햇살|볕뉘|서리|얼음|안개|파도|밀물|썰물|물때|"
    r"뿌리|줄기|가지|잎|꽃|열매|그루터기|낙엽|나무|풀[ ,.이은을]|"
    r"강아지|고양이|개미|거미줄|둥지|소[ ,.가와]|말[ ,.이 ]|"
    r"불씨|불티|잿더미|연기|그을음|모래|자갈|바위|흙|강[ ,.이은을]|"
    r"발자국|발[ ,.로이은을]|다리|손[ ,.이은을]|어깨|무릎|"
    r"눈[ ,.이은을 ]|길[ ,.이은을]")
THING = re.compile("|".join(
    p.pattern for p in (THING_NOW, THING_OLD, THING_LIVE)))

_FIG_SENT = re.compile(r"[^.?!]+[.?!]|[^.?!]+$")


def figure_hits(text: str) -> int:
    """그림이 몇 개인가 — **표지와 몸이 한 문장 안에** 있는 것만.

    「눌러 둔 용수철이 손을 떼면 튀는 것과 같소」 는 그림이고,
    「두 힘이 같이 걸린 것이오」 는 표지만입니다.
    """
    n = 0
    for s in _FIG_SENT.findall(text or ""):
        if FIGURE.search(s) and THING.search(s):
            n += 1
    return n
...
def score(sid: str, title: str, html: str, kind: str = "read",
    ...
    #   ★ 표지가 아니라 **그림**을 셉니다 (2026-09-17 · FIGURE 머리말).
    fig_hits = figure_hits(text)
