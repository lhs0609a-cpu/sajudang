...
def _ends_riul(ch: str) -> bool:
    """마지막 받침이 ㄹ인가. 「걸으오」(걷다)를 가려내는 자리요."""
    if not ch:
        return False
    o = ord(ch)
    if not (_BASE <= o <= _LAST):
        return False
    return (o - _BASE) % _JONG == 8          # ㄹ
...
# ── 어간이 「이」 로 끝나는 **움직씨** ──────────────────────
#
# ★ 「스스로를 몰아붙이오」 가 해요체에서 「몰아붙이에요」 로, 반말에서
#   「몰아붙이야」 로 나갔습니다 (2026-09-23).
#
#   아래 `_word` 는 이다(것이·일이)와 어간의 이(보이·쓰이)를 **앞
#   글자의 받침**으로 가릅니다 — 이다는 받침 있는 이름 뒤에 붙고
#   어간의 이는 받침 없는 글자 뒤에 온다는 자입니다. 대개는 맞는데,
#   시킴·입음으로 만든 어간(붙이 · 움직이 · 쌓이 · 묶이)은 앞 글자에
#   받침이 있어 **이름씨로 잘못 셉니다.**
#
#   ㅣ 어간에 -어가 붙으면 ㅕ로 줄어듭니다 (움직이+어 → 움직여).
#   여기 적힌 것만 그렇게 봅니다 — 자가 없는 낱말은 예전처럼 물러섭니다.
#   새 낱말이 새면 `tests/test_voice.py` 가 잡습니다.
_I_STEM = ("붙이", "움직이", "쌓이", "묶이", "기울이", "망설이",
           "섞이", "깎이", "놓이")
# ★ 꼬리로 보면 「돈벌이오」(이름씨) 까지 걸리므로 통째로 같을 때만.
_I_STEM_WHOLE = ("벌이",)
...
_YO_KEEP = ("네요", "세요", "예요", "에요", "아요", "어요", "여요", "워요",
            ...
            # ★ 「-ㄹ까요 · -을까요」 는 묻는 어미입니다 (2026-09-23).
            #   빠져 있어서 「끝낼까」 를 이름씨로 보고 「끝낼까입니다? ·
            #   끝낼까네? · 끝낼까예요?」 라는 **비문**을 냈습니다.
            #   하오체로 되돌릴 자가 없으니 그대로 둡니다 — 못 다루는
            #   꼴에서는 물러서는 것이 이 층의 규칙이오.
            "까요",
            "죠")
_NOUN = {
    HAPSYO: "입니다",
...
def _word(w: str, voice: str, ask: bool = False) -> str:
    ...
    # ★ 매개모음 「으」 를 뗍니다 (2026-09-23).
    #
    #   받침 있는 어간에 하오체를 붙이면 사이에 「으」 가 듭니다 —
    #   겪다 → 겪으오 · 잡다 → 잡으오. 그 「으」 는 어간이 아니라
    #   **이음소리**라, 다른 결로 갈 때는 떼고 붙여야 합니다.
    #   안 떼서 「혼자 겪읍니다 · 겪으네 · 겪으지 · 겪으네요」 가
    #   나갔습니다 — 시키는 말에서 이미 겪은 자리요(「끊으게」).
    #
    #   ★ 불규칙은 **물러섭니다.** 「걸으오」 는 걷다(ㄷ불규칙)라
    #     떼면 「걸습니다」 가 되어 도리어 틀립니다. 앞 글자 받침이
    #     ㄹ이면 되살릴 수가 없으니 손대지 않습니다.
    #     ㅅ불규칙(지으·나으)은 앞 글자에 받침이 없어 애초에 안 걸립니다.
    if (end == "오" and len(stem) >= 2 and stem[-1] == "으"
            and _has_batchim(stem[-2]) and not _ends_riul(stem[-2])):
        stem = stem[:-1]
        end = "소"                    # 받침 있는 어간 — 하오체로는 「소」
    ...
    if voice in (HAEYO, BANMAL) and not ask:
        ...
        if len(stem) >= 2 and stem[-1] == "이" and _has_batchim(stem[-2]):
            if stem.endswith(_I_STEM) or stem in _I_STEM_WHOLE:
                return stem[:-1] + ("여요" if voice == HAEYO else "여")
...
_JOSA = {"가": "이", "는": "은", "를": "을", "와": "과", "라": "이라",
         # ★ 「사주가 짚은 그대요.」 → 「당신요」 가 나갔습니다 (2026-09-11).
         #   문장 끝 「요」 는 이다(이오)의 준말이라 받침 뒤에서는 「이오」.
         "요": "이오"}
_YOU = re.compile(r"그대(?!로)([가는를와라]|요(?=[.!?…\s<]|$))?")
